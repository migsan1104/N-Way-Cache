...
**Do not regenerate `FPGA_Cache_PPA_Scaling_16KB.png` straight from the tree.** The README table
and chart are the 2026-08-20 sweep (Entries 1-8 RTL, all five on one revision). `timing.sh`
overwrites `cacheN.csv` and `outputs/` on every run, and `assoc_4`/`assoc_8` were re-run on
2026-08-23 with Entry-20-era RTL (407.3 / 383.6 MHz), so `collect_ppa.py` on the live tree
yields a mixed-RTL table (this was pushed by mistake on 2026-09-13 and reverted). The 08-20 rows
for A4/A8 are `git show 2028b76:openflex/PPA/assoc_N/cacheN.csv`; the matching power reports are
`power_assocN_2026-08-20_00*.rpt`. Either rebuild a scratch `--ppa-dir` from those, or re-run the
whole five-way sweep on one RTL revision and update the README table, prose and chart together.
