# ITERATION 6 - quadrant tiles with the iter-5 decongestion treatment, same
# die as v2/iter3 (the family's one advantage). Iter3 gated at 640k markers,
# jammed in the center-south cross where all four quadrant channels converge.
# Changes vs fp_v3a: way regions pulled 120 um back from both centerlines
# (was 20) so the central cross belongs to the hub; hub guide 840 -> 1040 um;
# pairs with ASIC_PLACE_MAX_DENSITY=0.62. Science question on record: does
# density relief rescue a structurally CONVERGENT floorplan, or only a
# locally starved one (iter5)? Sourced by 01_floorplan.tcl via ASIC_FLOORPLAN.
#
...
pnr_note "fp_iter6: die ${DIE_W}x${DIE_H}, margin $FP_CORE_MARGIN"
...
foreach w {0 1 2 3} {
    ...
    if {$ax eq "low"} { set rx0 [expr {$EDGE + 2*$MW + $GAPX + 30}]; set rx1 [expr {$DIE_W/2 - 120}] } \
    else              { set rx0 [expr {$DIE_W/2 + 120}]; set rx1 [expr {$DIE_W - ($EDGE + 2*$MW + $GAPX + 30)}] }
    if {$ay eq "low"} { set ry0 40.0; set ry1 [expr {$DIE_H/2 - 120}] } \
    else              { set ry0 [expr {$DIE_H/2 + 120}]; set ry1 [expr {$DIE_H - 40}] }
    createInstGroup way$w -region $rx0 $ry0 $rx1 $ry1
    addInstToInstGroup way$w "GEN_WAYS\[$w\]*"
    pnr_note "fp_iter6: way$w tile at ($x0,$ylow) region ($rx0,$ry0)-($rx1,$ry1)"
}
# hub: soft guide at the centre for the shared logic
set hx0 [expr {$DIE_W/2 - 520}] ; set hx1 [expr {$DIE_W/2 + 520}]
set hy0 [expr {$DIE_H/2 - 520}] ; set hy1 [expr {$DIE_H/2 + 520}]
...
pnr_note "fp_iter6: hub guide ($hx0,$hy0)-($hx1,$hy1)"
