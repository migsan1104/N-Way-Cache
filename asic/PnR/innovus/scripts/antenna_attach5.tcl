# antenna_attach5.tcl (2026-09-09): v4 with the router DIODE INSERTION ON, confined to the one selected net.
...
# Knobs: ASIC_ANTENNA_ATTACH_SRC (05_antenna_final.enc), ASIC_ANTENNA_NET,
...
set _vclk_out [file dirname [pnr_rpt antenna_attach5 io_vclk.txt]]
set io_vclk_applied 0
if {[catch {source [file join $_here io_vclk.tcl]} _m]} { puts "ATTACH5 io_vclk failed: $_m" }
proc ax {msg} { puts "ATTACH5 $msg"; pnr_note "ATTACH5 $msg" }
proc wns {tag} {
...
deselectAll
clearDrc
verify_drc -limit 100000 -report [pnr_rpt antenna_attach5 drc_final.rpt]
set d [llength [dbGet -e top.markers]]
set arpt [pnr_rpt antenna_attach5 antenna_final.rpt]
verifyProcessAntenna -report $arpt
set n [count_ant $arpt]
catch {checkPlace [pnr_rpt antenna_attach5 checkplace_final.rpt]}
foreach _r {
    {timeDesign -postRoute       -outDir [file dirname [pnr_rpt antenna_attach5 x]] -prefix final}
    {timeDesign -postRoute -hold -outDir [file dirname [pnr_rpt antenna_attach5 x]] -prefix final}
} { catch {eval $_r} }
wns "final"
ax "FINAL: verify_drc = $d, antenna = $n; checkpoint 05_antenna_final.enc"
saveDesign [pnr_ckpt 05_antenna_final5.enc]
exit
